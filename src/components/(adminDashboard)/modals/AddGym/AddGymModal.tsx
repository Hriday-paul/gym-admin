...
import { useAddGymMutation } from "@/redux/api/gym.api";
...
export function AddGymModal({
  ...
  const [handleAddGym, { isLoading }] = useAddGymMutation();
  ...
  const onFinish: FormProps<FieldType>['onFinish'] = async (values) => {
    ...
    try {
      ...
      values?.images.forEach(element => {
        formdata.append("images", element?.originFileObj as File)
      });

      await handleAddGym(formdata).unwrap();
      toast.success("New Gym added successfully");
      form.resetFields();
