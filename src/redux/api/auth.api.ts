...
const AuthApi = baseApi.injectEndpoints({
    endpoints: (builder) => ({
        ...
        loginAdmin: builder.mutation<{ message: string, data: { accessToken: string, refreshToken: string, user: IUser } }, { email: string, password: string }>({
            query: (data) => ({
...
export const { useRegisterUserMutation, useVerifyOtpMutation, useResendOtpMutation, useLoginAdminMutation, useResetPasswordMutation, useGetUserProfileQuery, useChangePasswordMutation, useUpdateProfileMutation, useForgotPasswordMutation } = AuthApi;